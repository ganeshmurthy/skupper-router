...
export SERVER_MYDOMAIN1=mydomain1.com
export SERVER_MYDOMAIN2=mydomain2.com
...
# Create a "bad" ca cert. This is actually a good cert but it will not be used to sign any
# of the other certs, so specifying it in place of ca cert will not work.
...
# Create a self signed server certificate (server-certificate.pem), sign it using the ca cert key (ca.pkcs12)
...
# Create a client certificate using the ca.pkcs12
...
# Concatenate the server cert and the ca cert into a single chained.pem file.
cat server-certificate.pem ca-certificate.pem > chained.pem

# Create the self-signed server cert for mydomain1.com
keytool $KEYARGS -keystore server-mydomain1.pkcs12 -storepass server-password -alias server-certificate-mydomain1 -keypass server-password -genkey  -dname "O=Server,CN=mydomain1.com" -validity 99999
keytool $KEYARGS -keystore server-mydomain1.pkcs12 -storepass server-password -alias server-certificate-mydomain1 -keypass server-password -certreq -file server-request-mydomain1.pem
keytool $KEYARGS -keystore ca.pkcs12 -storepass ca-password -alias ca -keypass ca-password -gencert -rfc -validity 99999 -infile server-request-mydomain1.pem -outfile server-certificate-mydomain1.pem
# Create a server private key without a password
openssl pkcs12 -nocerts -passin pass:server-password -in server-mydomain1.pkcs12 -nodes -out server-private-key-mydomain1-no-pass.pem
# Concatenate server-certificate-mydomain1.pem and ca cert into a single chained.pem file.
cat server-certificate-mydomain1.pem ca-certificate.pem > chained-mydomain1.pem


# Create the self-signed server cert for mydomain2.com
keytool $KEYARGS -keystore server-mydomain2.pkcs12 -storepass server-password -alias server-certificate-mydomain2 -keypass server-password -genkey  -dname "O=Server,CN=mydomain2.com" -validity 99999
keytool $KEYARGS -keystore server-mydomain2.pkcs12 -storepass server-password -alias server-certificate-mydomain2 -keypass server-password -certreq -file server-request-mydomain2.pem
keytool $KEYARGS -keystore ca.pkcs12 -storepass ca-password -alias ca -keypass ca-password -gencert -rfc -validity 99999 -infile server-request-mydomain2.pem -outfile server-certificate-mydomain2.pem
# Create a server private key without a password
openssl pkcs12 -nocerts -passin pass:server-password -in server-mydomain2.pkcs12 -nodes -out server-private-key-mydomain2-no-pass.pem
# Concatenate server-certificate-mydomain2.pem and ca cert into a single chained.pem file.
cat server-certificate-mydomain2.pem ca-certificate.pem > chained-mydomain2.pem
