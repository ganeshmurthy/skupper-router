 ...
 *
 * @return QD_TLS_ERROR if there is an error when trying to decrypt an encrypted buffer, the number of decrypted bytes
 * in total otherwise
 *
 */
 ...
 *
 *
 * @Returns true if there is no error when trying to encrypt an unencrypted buffer or there was no trouble during
