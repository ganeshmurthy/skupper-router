 ...
 * Stream status
 */
typedef enum { QD_STREAM_OPEN, QD_STREAM_HALF_CLOSED, QD_STREAM_FULLY_CLOSED } qd_http2_stream_status_t;
...
struct qdr_http2_stream_data_t {
    ...
    bool out_msg_send_complete;      // we use this flag to save the send_complete flag because the delivery and message
                                     // associated with this stream might have been freed.
    ...
    bool header_and_props_composed;  // true if the header and properties of the inbound message have already been
                                     // composed so we don't have to do it again.
...
struct qdr_http2_connection_t {
    ...
    time_t last_pn_raw_conn_read;  // The last time a PN_RAW_CONNECTION_READ event was invoked with more than zero bytes
                                   // on an egress connection.
    ...
    bool delete_egress_connections;  // If set to true, the egress qdr_connection_t and qdr_http2_connection_t objects
                                     // will be deleted
