...
static void handle_outgoing_tls(qdr_http2_connection_t *conn, const pn_raw_buffer_t *outgoing_buff,
                                size_t outgoing_buff_size, bool can_write_buffers);
...
/**
 * If an ALPN protocol was detected by the TLS API, we make sure that the protocol matches the "h2" (short for http2)
 * protocol. The connection is simply closed if any other protocol (other than h2) was detected. It is ok for no
 * protocol to be detected which means that the other side might not be doing ALPN. If this is the case, we still
 * continue sending http2 frames and close the connection if the response to those http2 frames is non-http2.
 *
 */
static void check_alpn_protocol(qdr_http2_connection_t *http_conn)
{
    ...
    if (pn_tls_get_alpn_protocol(http_conn->tls_session, &protocol_name, &protocol_name_length)) {
        ...
        qd_log(http2_adaptor->log_source, QD_LOG_TRACE, "[C%" PRIu64 "] Using protocol %s obtained via ALPN",
        ...
        if (strcmp(alpn_protocol, protocols[0])) {
            ...
            nghttp2_submit_goaway(http_conn->session, 0, 0, NGHTTP2_PROTOCOL_ERROR, (uint8_t *) "TLS Protocol Error",
                                  18);
        ...
        //
        qd_log(http2_adaptor->log_source, QD_LOG_TRACE, "[C%" PRIu64 "] No ALPN protocol was returned",
               http_conn->conn_id);
    }
...
static void advance_stream_status(qdr_http2_stream_data_t *stream_data)
{
    qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
           "[C%" PRIu64 "][S%" PRId32 "] Trying to move stream status", stream_data->conn->conn_id,
           stream_data->stream_id);
    if (stream_data->status == QD_STREAM_OPEN) {
        ...
        qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE, "[C%" PRIu64 "][S%" PRId32 "] Unknown stream status",
               stream_data->conn->conn_id, stream_data->stream_id);
    }
}
...
static size_t write_buffers(qdr_http2_connection_t *conn)
{
    ...
    qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
           "[C%" PRIu64 "] write_buffers pn_raw_connection_write_buffers_capacity=%zu", conn->conn_id,
           pn_buffs_to_write);
    ...
    if (num_buffs == 0) {
        ...
        qd_log(http2_adaptor->log_source, QD_LOG_TRACE,
               "[C%" PRIu64
               "] Written 0 buffers in write_buffers() - pn_raw_connection_write_buffers_capacity = %zu, "
               "DEQ_SIZE(conn->buffs) = %zu - returning",
               conn->conn_id, pn_buffs_to_write, DEQ_SIZE(conn->buffs));
        return num_buffs;
    }
    ...
    qd_log(http2_adaptor->log_source, QD_LOG_TRACE,
           "[C%" PRIu64
           "] Written %zu buffer(s) and %i bytes in write_buffers() using pn_raw_connection_write_buffers()",
           conn->conn_id, num_buffers_written, total_bytes);
...
static void free_http2_stream_data(qdr_http2_stream_data_t *stream_data, bool on_shutdown)
{
    ...
    // If the httpConnector was deleted, a client request has nowhere to go because of lack of receiver and hence
    // credit. No delivery was created. The message that was created for such a hanging request must be freed here..
...
void free_qdr_http2_connection(qdr_http2_connection_t *http_conn, bool on_shutdown)
{
    ...
    //
    // We are about to free the qdr_http2_connection_t object. We need to decref the listener/connector, so they can be
    // freed.
    //
    if (http_conn->listener) {
        qd_http_listener_decref(http_conn->listener);
    } else if (http_conn->connector) {
        // Note here that decrefing the connector also frees the config. The http_conn->config remains unfreed and
        // accessible on the qdr_http2_connection_t object until we call the following qd_http_connector_decref which
        // *might* then free the config and then free connector itself.
...
static qdr_http2_stream_data_t *create_http2_stream_data(qdr_http2_connection_t *conn, int32_t stream_id)
{
    ...
    qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
           "[C%" PRIu64 "][S%" PRId32 "] Created new stream_data (%lx)", conn->conn_id, stream_id, (long) stream_data);
...
/**
 * This callback function  is invoked when the nghttp2 library tells the application about the error code, and error
 * message.
 */
...
/**
 * Callback function invoked by nghttp2_session_recv() and nghttp2_session_mem_recv() when an invalid non-DATA frame is
 * received
 */
static int on_invalid_frame_recv_callback(nghttp2_session *session, const nghttp2_frame *frame, int lib_error_code,
                                          void *user_data)
{
    qdr_http2_connection_t *conn      = (qdr_http2_connection_t *) user_data;
    int32_t                 stream_id = frame->hd.stream_id;
    qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
           "[C%" PRIu64 "][S%" PRId32 "] on_invalid_frame_recv_callback, lib_error_code=%i", conn->conn_id, stream_id,
           lib_error_code);
    ...
    nghttp2_session *session, uint8_t flags, int32_t stream_id, const uint8_t *data, size_t len, void *user_data)
{
    ...
    //
    // DISPATCH-1868: If an in_dlv is present it means that the qdr_link_deliver() has already been called (delivery has
    // already been routed) in which case qd_message_stream_data_append can be called to append buffers to the message
    // body If stream_data->in_dlv = 0 but stream_data->header_and_props_composed is true, it means that the message has
    // not been routed yet but the message already has headers and properties in which case the
    // qd_message_stream_data_append() can be called to add body data to the message. In many cases when the response
    // message is streamed by a server, the entire message body can arrive before we get credit to route it. We want to
    // be able to keep collecting the incoming DATA in the message object so we can ultimately route it when the credit
    // does ultimately arrive.
    //
    if (stream_data->in_dlv || stream_data->header_and_props_composed) {
        qd_buffer_list_t buffers;
        DEQ_INIT(buffers);
        qd_buffer_list_append(&buffers, (uint8_t *) data, len);
        // DISPATCH-1868: Part of the HTTP2 message body arrives *before* we can route the delivery. So we accumulated
        // the body buffers in the stream_data->body_buffers. But before the rest of the HTTP2 data arrives, we got
        // credit to send the delivery and we have an in_dlv object now. Now, we take the buffers that were added
        // previously to stream_data->body_buffers and call qd_message_stream_data_append
        ...
        if ((q2_blocked1 || q2_blocked2) && !conn->q2_blocked) {
            ...
            qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE, "[C%" PRIu64 "] q2 is blocked on this connection",
                   conn->conn_id);
        }
    } else {
        ...
        qd_log(
            http2_adaptor->protocol_log_source, QD_LOG_TRACE,
            "[C%" PRIu64 "][S%" PRId32
                              ...
                              void                *user_data)
{
    ...
    qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
           "[C%" PRIu64 "][S%" PRId32 "] send_data_callback length=%zu", conn->conn_id, stream_data->stream_id, length);
    ...
    if (require_tls) {
        ...
        if (num_local_buffs > 0) {
            ...
            qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
                   "[C%" PRIu64 "][S%" PRId32 "] send_data_callback require_tls, num_local_buffs=%zu", conn->conn_id,
                   stream_data->stream_id, num_local_buffs);
...
static ssize_t send_callback(nghttp2_session *session, const uint8_t *data, size_t length, int flags, void *user_data)
{
    ...
    qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE, "[C%" PRIu64 "] HTTP2 send_callback data length %zu",
           conn->conn_id, length);
 ...
 * The HEADERS frame can arrive from a client or server. We start building a new AMQP message (qd_message_t) in this
 * callback and create the two links per stream.
 *
...
static bool compose_and_deliver(qdr_http2_connection_t *conn, qdr_http2_stream_data_t *stream_data,
...
static void create_settings_frame(qdr_http2_connection_t *conn)
{
    ...
    if (rv != 0) {
        qd_log(http2_adaptor->log_source, QD_LOG_ERROR, "[C%" PRIu64 "] Fatal error sending settings frame, rv=%i",
               conn->conn_id, rv);
        return;
    }
    qd_log(http2_adaptor->log_source, QD_LOG_TRACE, "[C%" PRIu64 "] Initial SETTINGS frame sent on conn=%p",
...
static void _http_record_request(qdr_http2_connection_t *conn, qdr_http2_stream_data_t *stream_data)
{
    ...
    qd_http_record_request(http2_adaptor->core, stream_data->method,
                           ...
                           conn->config ? conn->config->adaptor_config->address : 0, remote_addr,
                           conn->config ? conn->config->adaptor_config->site_id : 0, stream_data->remote_site,
...
static int on_frame_recv_callback(nghttp2_session *session, const nghttp2_frame *frame, void *user_data)
{
    ...
    switch (frame->hd.type) {
        case NGHTTP2_GOAWAY: {
            //
            // A GOAWAY frame has been received from the HTTP2 server. Usually a server sends a GOAWAY but nothing
            // prevents the client from sending one.
            //
            ...
            // previously established streams.  This enables administrative actions, like server maintenance. Receivers
            // of a GOAWAY frame MUST NOT open additional streams on the connection, although a new connection can be
            // established for new streams."
            //
            // We will close any unprocessed streams on the connection. In doing so, all the outstanding deliveries on
            // that connection will be PN_RELEASED which will in turn release all the peer deliveries on the client side
            // which will enable us to send a GOAWAY frame to the client. This is how we propagate a GOAWAY received
            // from the server side to the client side.
            //
            // We will also close the pn_raw_connection (we will not close the qdr_connection_t and the
            // qdr_http2_connection_t, those will still remain). This will close the TCP connection to the server and
            // will enable creation  of a new connection to the server since we are not allowed to create any more
            // streams on the connection that received the GOAWAY frame.
            //
            ...
            // Free all streams that are greater that the last_stream_id because the server is not going to process
            // those streams.
        ...
        case NGHTTP2_PING: {
            qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
                   "[C%" PRIu64 "][S%" PRId32 "] HTTP2 PING frame received", conn->conn_id, stream_id);
        } break;
        case NGHTTP2_PRIORITY: {
            qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
                   "[C%" PRIu64 "][S%" PRId32 "] HTTP2 PRIORITY frame received", conn->conn_id, stream_id);
        } break;
        case NGHTTP2_SETTINGS: {
            qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
                   "[C%" PRIu64 "][S%" PRId32 "] HTTP2 SETTINGS frame received", conn->conn_id, stream_id);
        } break;
        ...
        case NGHTTP2_DATA: {
            ...
            qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
                   "[C%" PRIu64 "][S%" PRId32 "] NGHTTP2_DATA frame received", conn->conn_id, stream_id);
            ...
            if (stream_data->out_dlv && !stream_data->disp_updated && !stream_data->out_dlv_decrefed
                && stream_data->status == QD_STREAM_FULLY_CLOSED) {
        ...
        case QD_MESSAGE_DEPTH_OK: {
            ...
            switch (stream_data->curr_stream_data_result) {
                case QD_MESSAGE_STREAM_DATA_BODY_OK: {
                    ...
                    qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
                           "[C%" PRIu64 "][S%" PRId32
                           "] read_data_callback QD_MESSAGE_STREAM_DATA_BODY_OK "
                           "pn_raw_connection_write_buffers_capacity=%zu",
                    ...
                    if (pn_buffs_write_capacity == 0) {
                        ...
                        qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
                               "[C%" PRIu64 "][S%" PRId32
                               "] Exiting read_data_callback, QD_MESSAGE_STREAM_DATA_BODY_OK "
                               "pn_buffs_write_capacity=0, pausing stream, returning NGHTTP2_ERR_DEFERRED",
                               conn->conn_id, stream_data->stream_id);
                    ...
                    if (payload_length == 0) {
                        ...
                        //
                        // The payload length on this body data is zero. Nothing to do, just return zero to move on to
                        // the next body data. Usually, zero length body datas are a result of programmer error.
                        //
                        qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
                    ...
                    if (payload_length) {
                        ...
                        qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
                               "[C%" PRIu64 "][S%" PRId32
                               "] read_data_callback remaining_payload_length=%i, length=%zu",
                        ...
                        if (remaining_payload_length <= QD_HTTP2_BUFFER_SIZE) {
                            if (length < remaining_payload_length) {
                                ...
                                qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
                                       "[C%" PRIu64 "][S%" PRId32
                                       "] read_data_callback remaining_payload_length (%i) <= "
                                ...
                                if (stream_data->next_stream_data_result == QD_MESSAGE_STREAM_DATA_NO_MORE) {
                                    *data_flags |= NGHTTP2_DATA_FLAG_EOF;
                                    ...
                                    qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
                                           "[C%" PRIu64 "][S%" PRId32
                                           "] read_data_callback, looking ahead one body data "
                                           "QD_MESSAGE_STREAM_DATA_NO_MORE",
                                           conn->conn_id, stream_data->stream_id);
                                } else if (stream_data->next_stream_data_result == QD_MESSAGE_STREAM_DATA_FOOTER_OK) {
                            ...
                            // We can only send what nghttp2 allows us to send. nghttp2 might be doing http2 flow
                            // control and we abide by it.
                            ...
                            qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
                                   "[C%" PRIu64 "][S%" PRId32
                                   "] read_data_callback remaining_payload_length <= QD_HTTP2_BUFFER_SIZE ELSE "
                                   "bytes_to_send=%zu",
                ...
                case QD_MESSAGE_STREAM_DATA_NO_MORE: {
                    ...
                    if (pn_buffs_write_capacity == 0) {
                        ...
                        qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
                               "[C%" PRIu64 "][S%" PRId32
                               "] read_data_callback QD_MESSAGE_STREAM_DATA_NO_MORE - pn_buffs_write_capacity=0 send "
                               "is not complete",
                               conn->conn_id, stream_data->stream_id);
                        return NGHTTP2_ERR_DEFERRED;
                    } else {
                        *data_flags |= NGHTTP2_DATA_FLAG_EOF;
                        ...
                        if (stream_data->out_msg_has_footer) {
                            ...
                            *data_flags |= NGHTTP2_DATA_FLAG_NO_END_STREAM;
                            qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
                                   "[C%" PRIu64 "][S%" PRId32
                                   "] read_data_callback stream_data->out_msg_has_footer, setting "
                        ...
                        qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
                               "[C%" PRIu64 "][S%" PRId32
                               "] read_data_callback QD_MESSAGE_STREAM_DATA_NO_MORE - "
                               "stream_data->out_dlv_local_disposition = PN_ACCEPTED - send_complete=true, setting "
...
qdr_http2_connection_t *qdr_http_connection_ingress(qd_http_listener_t *listener)
{
    ...
    nghttp2_session_server_new(&(ingress_http_conn->session), (nghttp2_session_callbacks *) http2_adaptor->callbacks,
                               ingress_http_conn);
...
static void grant_read_buffers(qdr_http2_connection_t *conn)
{
    ...
    if (conn->pn_raw_conn) {
        ...
        while (desired) {
            ...
            desired -= i;
            qd_log(http2_adaptor->log_source, QD_LOG_TRACE,
                   "[C%" PRIu64 "] Calling pn_raw_connection_give_read_buffers in grant_read_buffers", conn->conn_id);
...
static void qdr_http_delivery_update(void *context, qdr_delivery_t *dlv, uint64_t disp, bool settled)
{
    ...
    // the router ends up forwarding all the data to the curl client without sending the END_STREAM to the client. The
    // END_STREAM does arrive from the server but not before the curl client closes the client connection after
    // receiving all the data. The curl client does not wait for the router to send an END_STREAM flag to close the
    // connection. The client connection closure triggers the link cleanup on the ingress connection, in turn freeing up
    // all deliveries and its peer deliveries. The peer delivery is released while it is still receiving the END_STREAM
    // frame and the router crashes when we try to set receive complete on the message because the message has already
    // been freed. To solve this issue, the stream_data->stream_force_closed flag is set to true when the peer delivery
    // is released and this flag is check when performing further actions on the delivery. No action on the peer
    // delivery is performed if this flag is set because the delivery and its underlying message have been freed.
    //
    ...
    if (settled) {
        ...
        if (conn->ingress && (disp == PN_RELEASED || disp == PN_MODIFIED || disp == PN_REJECTED)) {
            ...
            nghttp2_submit_headers(stream_data->conn->session, NGHTTP2_FLAG_END_HEADERS | NGHTTP2_FLAG_END_STREAM,
                                   stream_data->stream_id, NULL, hdrs, 3, 0);
        }

        if (!conn->ingress && (disp == PN_RELEASED || disp == PN_MODIFIED || disp == PN_REJECTED)) {
            //
            // On the server side connection, send a DATA frame with an END_STREAM flag thus closing the particular
            // stream. We don't want to close the entire connection like we did not the client side.
            //
            nghttp2_submit_data(conn->session, NGHTTP2_FLAG_END_STREAM, stream_data->stream_id, &conn->data_prd);
            // On the ingress side, there is a possibility that the client immediately closes the connection as soon as
            // it receives the required data frames. The client does not need to wait to receive the END_STREAM flag.
            // When that happens, the delivery on the ingress side is freed which in turn releases its peer delivery
            // which means we can close the stream on the egress side. The stream status can be set to
            // QD_STREAM_FULLY_CLOSED and freed.
            advance_stream_status(stream_data);
...
static void qdr_http_first_attach(void *context, qdr_connection_t *conn, qdr_link_t *link, qdr_terminus_t *source,
                                  qdr_terminus_t *target, qd_session_class_t session_class)
{
}
...
static void qdr_http_second_attach(void *context, qdr_link_t *link, qdr_terminus_t *source, qdr_terminus_t *target)
{
    ...
    if (stream_data) {
        if (qdr_link_direction(link) == QD_OUTGOING && source->dynamic) {
            if (stream_data->conn->ingress) {
                ...
                if (route_delivery(stream_data, qd_message_receive_complete(stream_data->message))) {
                    qd_log(http2_adaptor->log_source, QD_LOG_TRACE,
                           "[C%" PRIu64 "] Reply-to available now, delivery routed successfully",
...
static void qdr_http_activate(void *notused, qdr_connection_t *c)
{
    ...
    if (conn) {
        if (conn->pn_raw_conn
            && !(IS_ATOMIC_FLAG_SET(&conn->raw_closed_read) && IS_ATOMIC_FLAG_SET(&conn->raw_closed_write))) {
            qd_log(http2_adaptor->log_source, QD_LOG_TRACE,
            ...
            qd_log(http2_adaptor->log_source, QD_LOG_INFO,
...
{
    sys_mutex_lock(qd_server_get_activation_lock(http2_adaptor->core->qd->server));
    qd_log(http2_adaptor->log_source, QD_LOG_INFO, "[C%" PRIu64 "] Connecting to %s", conn->conn_id,
    ...
    if (conn->require_tls) {
        ...
        if (tls_setup_success) {
            ...
            qd_log(http2_adaptor->log_source, QD_LOG_ERROR, "[C%" PRIu64 "] Error setting up TLS on connector %s to %s",
...
uint64_t handle_outgoing_http(qdr_http2_stream_data_t *stream_data)
{
    ...
    qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE, "[C%" PRIu64 "] Starting to handle_outgoing_http",
           conn->conn_id);
    if (stream_data->out_dlv) {
        ...
        if (!stream_data->out_msg_header_sent) {
            qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE, "[C%" PRIu64 "] Header not sent yet",
                   conn->conn_id);
            ...
            stream_data->stream_id =
                nghttp2_submit_headers(conn->session, flags, stream_id, NULL, hdrs, count, stream_data);
            ...
            qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE, "[C%" PRIu64 "][S%" PRId32 "] Headers submitted",
                   conn->conn_id, stream_data->stream_id);
        ...
        if (stream_data->out_msg_has_body) {
            if (stream_data->out_msg_header_sent) {
                // This is usually called if there are many AMQP data streams objects in a delivery. These data streams
                // were created on the inbound AMQP side using the qdr_delivery_continue() function.
                ...
                qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
                       "[C%" PRIu64 "][S%" PRId32 "] Processing message body", conn->conn_id, stream_data->stream_id);
                conn->data_prd.source.ptr = stream_data;

                int rv = 0;
                rv     = nghttp2_submit_data(conn->session, NGHTTP2_FLAG_END_STREAM, stream_data->stream_id,
                                             &conn->data_prd);
                if (rv != 0) {
                    qd_log(http2_adaptor->protocol_log_source, QD_LOG_ERROR,
            ...
            qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE, "[C%" PRIu64 "][S%" PRId32 "] Message has no body",
                   conn->conn_id, stream_data->stream_id);
        }
        ...
        if (stream_data->out_msg_has_footer) {
            qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
                   "[C%" PRIu64 "][S%" PRId32 "] Message has a footer", conn->conn_id, stream_data->stream_id);
            bool send_footer = false;
            if (stream_data->out_msg_has_body && !stream_data->out_msg_body_sent) {
                if (stream_data->out_msg_body_sent) {
                    send_footer = true;
                    qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
                           "[C%" PRIu64 "][S%" PRId32 "] send_footer = true", conn->conn_id, stream_data->stream_id);
                } else {
                    qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
                           "[C%" PRIu64 "][S%" PRId32 "] send_footer = false", conn->conn_id, stream_data->stream_id);
                }
            } else {
                send_footer = true;
            }
            ...
            if (send_footer) {
                qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
                       "[C%" PRIu64 "][S%" PRId32 "] Starting to send footer", conn->conn_id, stream_data->stream_id);
                ...
                nghttp2_submit_headers(
                    conn->session, NGHTTP2_FLAG_END_STREAM, stream_data->stream_id, NULL, hdrs, count, stream_data);
            ...
            qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
                   "[C%" PRIu64 "][S%" PRId32 "] Message has no footer", conn->conn_id, stream_data->stream_id);
        }
        ...
        qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE, "[C%" PRIu64 "] Finished handle_outgoing_http",
               conn->conn_id);
    } else {
        qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE, "[C%" PRIu64 "] No out_dlv, no handle_outgoing_http",
               conn->conn_id);
    }
    return 0;
}
...
static bool push_rx_buffer_to_nghttp2(qdr_http2_connection_t *conn, uint8_t *buf, size_t size)
{
    ...
    qd_log(http2_adaptor->log_source, QD_LOG_DEBUG,
           ...
           "qd_http2_buffer of size %" PRIu32 " ",
           conn->conn_id, size);
    ...
    if (rv < 0) {
        ...
        if (rv == NGHTTP2_ERR_FLOODED) {
            ...
            qd_log(http2_adaptor->protocol_log_source, QD_LOG_ERROR, "[C%" PRIu64 "] HTTP NGHTTP2_ERR_FLOODED",
                   conn->conn_id);
            ...
            qd_log(http2_adaptor->protocol_log_source, QD_LOG_ERROR, "[C%" PRIu64 "] HTTP NGHTTP2_ERR_CALLBACK_FAILURE",
                   conn->conn_id);
...
static bool process_tls(qdr_http2_connection_t *conn)
{
    ...
    if (err && !conn->tls_error) {
        ...
        qd_log(http2_adaptor->protocol_log_source, QD_LOG_ERROR,
               "[C%" PRIu64 "] conn->ingress=%i, Error processing TLS: %s", conn->conn_id, conn->ingress, error_msg);
...
/**
 * Encrypts all outgoing data and writes the encrypted data immediately out to the wire based on
 * can_write_result_buffers flag. Encrypts data going out from router to http2 server (when the router is acting as a
 * client and also encrypts data that is going out from router to http2 client. Handles handshake when acting as a
 * client.
 *
 */
static void handle_outgoing_tls(qdr_http2_connection_t *conn, const pn_raw_buffer_t *unencrypted_buffs,
                                size_t unencrypted_buff_count, bool can_write_result_buffers)
{
    if (conn->tls_error)
        return;
    ...
    if (pn_tls_is_secure(conn->tls_session)) {
        qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
               "[C%" PRIu64 "] HTTP2 handle_outgoing_tls pn_tls_can_encrypt(conn->tls_session)", conn->conn_id);
        ...
        if (!conn->ingress) {
            if (!conn->handled_connected_event) {
                qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
                       "[C%" PRIu64 "] HTTP2 handle_outgoing_tls calling handle_raw_connected_event", conn->conn_id);
        ...
        while (processed_unencrypted_buffs_count < unencrypted_buff_count && unencrypted_buff_count) {
            ...
            size_t num_buffs_to_process =
                MIN(encrypt_input_buff_capacity, unencrypted_buff_count - processed_unencrypted_buffs_count);
            if (num_buffs_to_process == 0)
                break;

            size_t consumed = pn_tls_give_encrypt_input_buffers(
                conn->tls_session, &unencrypted_buffs[processed_unencrypted_buffs_count], num_buffs_to_process);
    ...
    while (pn_tls_need_encrypt_output_buffers(conn->tls_session)) {
        ...
        size_t encrypt_result_buffers_count =
            pn_tls_give_encrypt_output_buffers(conn->tls_session, &encrypted_result_raw_buffer, 1);
        ...
        qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
               "[C%" PRIu64 "] HTTP2 handle_outgoing_tls process_tls successful", conn->conn_id);

        pn_raw_buffer_t take_encrypted_result_buffer;
        size_t          take_encrypted_result_buffers_count =
            pn_tls_take_encrypt_output_buffers(conn->tls_session, &take_encrypted_result_buffer, 1);
...
{
    qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
           "[C%" PRIu64 "] HTTP2 handle_incoming_tls incoming_buf.size=%zu", conn->conn_id, incoming_buf->size);
    ...
    if (result_decrypt_buff_capacity == 0) {
        qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
               "[C%" PRIu64 "] HTTP2 handle_incoming_tls result_decrypt_buff_capacity == 0", conn->conn_id);
    } else {
        //
        // Process TLS.
        //
        ...
        qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
               "[C%" PRIu64 "] HTTP2 handle_incoming_tls process_tls successful", conn->conn_id);
        ...
        while (pn_tls_need_decrypt_output_buffers(conn->tls_session)) {
            ...
            qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
                   "[C%" PRIu64 "] HTTP2 handle_incoming_tls process_tls successful, result=%zu", conn->conn_id,
                   result);
        }
        ...
        while (pn_tls_take_decrypt_output_buffers(conn->tls_session, &take_decrypted_output_buff, 1)) {
            ...
            close_conn = push_rx_buffer_to_nghttp2(conn, (uint8_t *) take_decrypted_output_buff.bytes,
                                                   take_decrypted_output_buff.size);
            qd_http2_buffer_t *take_decrypt_output_http2_buff =
                (qd_http2_buffer_t *) take_decrypted_output_buff.context;
    ...
    qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
           "[C%" PRIu64 "] HTTP2 handle_incoming_tls conn->tls_has_output=%i", conn->conn_id, conn->tls_has_output);
...
static int handle_incoming_http(qdr_http2_connection_t *conn)
{
    ...
    // To remedy this problem, when nodejs sends the initial SETTINGS frame, we don't tell nghttp2 about it. So step 2c
    // happens before step 2b and nodejs is now happy
    //
...
static void restart_streams(qdr_http2_connection_t *http_conn)
{
    qdr_http2_stream_data_t *stream_data = DEQ_HEAD(http_conn->streams);
    if (!stream_data) {
        qd_log(http2_adaptor->log_source, QD_LOG_TRACE, "[C%" PRIu64 "] In restart_streams, no stream_data, returning",
               http_conn->conn_id);
        return;
    }
...
static void qdr_del_http2_connection_CT(qdr_core_t *core, qdr_action_t *action, bool discard)
{
    ...
    // This means that the qdr_adaptors_finalize has already been called and the connection in question has already been
    // freed. No need to do anything now, if discard, just return.
    //
    if (discard)
        return;
...
static void close_connections(qdr_http2_connection_t *conn)
{
    if (conn->dummy_link) {
        ...
        qd_log(http2_adaptor->log_source, QD_LOG_TRACE, "[C%" PRIu64 "] Detaching dummy link on egress connection",
               conn->conn_id);
        conn->dummy_link = 0;
    }
...
static void create_stream_dispatcher_link(qdr_http2_connection_t *egress_http_conn)
{
    ...
    // The core frees all links before it calls adaptor final. so we cannot get the stream data from the qdr_link
    // context.
...
qdr_http2_connection_t *qdr_http_connection_egress(qd_http_connector_t *connector)
{
    ...
    qdr_connection_info_t *info =
        qdr_connection_info(false,                                                // bool             is_encrypted,
                            false,                                                // bool             is_authenticated,
                            true,                                                 // bool             opened,
                            "",                                                   // char            *sasl_mechanisms,
...
static void handle_raw_connected_event(qdr_http2_connection_t *conn)
{
    ...
    if (conn->ingress) {
        ...
        qd_log(log, QD_LOG_INFO, "[C%" PRIu64 "] Accepted Ingress ((PN_RAW_CONNECTION_CONNECTED)) from %s",
               conn->conn_id, conn->remote_address);
    } else {
        CLEAR_ATOMIC_FLAG(&conn->raw_closed_read);
        CLEAR_ATOMIC_FLAG(&conn->raw_closed_write);
        if (!conn->session) {
            nghttp2_session_client_new(&conn->session, (nghttp2_session_callbacks *) http2_adaptor->callbacks,
                                       (void *) conn);
            send_settings_frame(conn);
            conn->client_magic_sent = true;
        ...
        qd_log(log, QD_LOG_TRACE, "[C%" PRIu64 "] Created stream dispatcher link in PN_RAW_CONNECTION_CONNECTED",
               conn->conn_id);
    ...
    switch (pn_event_type(e)) {
        ...
        case PN_RAW_CONNECTION_CLOSED_READ: {
            if (conn->q2_blocked) {
                ...
                qd_log(http2_adaptor->protocol_log_source, QD_LOG_TRACE,
                       "[C%" PRIu64 "] q2 is unblocked on this connection (PN_RAW_CONNECTION_CLOSED_READ)",
                       conn->conn_id);
            }
        ...
        case PN_RAW_CONNECTION_NEED_WRITE_BUFFERS: {
            qd_log(log, QD_LOG_TRACE, "[C%" PRIu64 "] PN_RAW_CONNECTION_NEED_WRITE_BUFFERS Need write buffers",
                   conn->conn_id);
            break;
        }
        case PN_RAW_CONNECTION_NEED_READ_BUFFERS: {
            qd_log(log, QD_LOG_TRACE, "[C%" PRIu64 "] PN_RAW_CONNECTION_NEED_READ_BUFFERS Need read buffers",
                   conn->conn_id);
            grant_read_buffers(conn);
            break;
        }
        ...
        case PN_RAW_CONNECTION_WRITTEN: {
            ...
            qd_log(log, QD_LOG_TRACE,
                   "[C%" PRIu64 "] PN_RAW_CONNECTION_WRITTEN Wrote %zu bytes, DEQ_SIZE(conn->buffs) = %zu",
                   conn->conn_id, written, DEQ_SIZE(conn->buffs));
...
{
    if (connector) {
        qd_log(http2_adaptor->log_source, QD_LOG_INFO, "Deleted HttpConnector for %s, %s:%s",
 ...
 */
qd_http_listener_t *qd_http2_configure_listener(qd_dispatch_t *qd, qd_http_adaptor_config_t *config,
                                                qd_entity_t *entity)
{
...
qd_http_connector_t *qd_http2_configure_connector(qd_dispatch_t *qd, qd_http_adaptor_config_t *config,
                                                  qd_entity_t *entity)
{
...
static void qdr_http2_adaptor_init(qdr_core_t *core, void **adaptor_context)
{
    ...
    *adaptor_context = adaptor;
    ...
    //
    // These callbacks are called when we feed the incoming binary http2 data from the client or the server to
    // nghttp2_session_mem_recv() in handle_incoming_http
    //
