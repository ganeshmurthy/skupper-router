...
static bool check_alpn_protocol_match(qdr_http2_connection_t *http_conn)
{
    ...
    if (pn_tls_get_alpn_protocol(http_conn->tls->tls_session, &protocol_name, &protocol_name_length)) {
        //
...
static uint64_t qdr_http_deliver(void *context, qdr_link_t *link, qdr_delivery_t *delivery, bool settled)
{
    ...
    if (!stream_data) {
        qd_log(http2_adaptor->log_source, QD_LOG_DEBUG, DLV_FMT " qdr_http_deliver, no stream data, returning",
               DLV_ARGS(delivery));
        return 0;
    }
...
static bool push_rx_buffer_to_nghttp2(qdr_http2_connection_t *conn, uint8_t *buf, size_t size)
{
    ...
    qd_log(http2_adaptor->protocol_log_source, QD_LOG_DEBUG,
                  "[C%"PRIu64"] nghttp2_session_mem_recv rv=%i", conn->conn_id, rv);
    ...
    if (rv < 0) {
        ...
        if (rv == NGHTTP2_ERR_FLOODED) {
            ...
            nghttp2_submit_goaway(conn->session, 0, 0,
                    NGHTTP2_ERR_FRAME_SIZE_ERROR, (uint8_t *)"Bad Frame Size", 14);
        }
...
static int handle_incoming_http(qdr_http2_connection_t *conn)
{
    ...
    if (conn->require_tls) {
        ...
        output = qd_tls_decrypt(conn->tls, conn->pn_raw_conn, &decrypted_buffs, &conn->granted_read_buffs);
        if (output == QD_TLS_ERROR) {
            pn_raw_connection_close(conn->pn_raw_conn);
            return 0;
        ...
        conn->encrypted_bytes_in += output;
    } else {
        //
